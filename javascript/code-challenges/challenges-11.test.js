...
const count = (target, input) => {
 let count =0;
 input.map(item=>{
   item.map(elment => elment === target ? count++ : count)
 })
 return count;
};
...
let findShortest = (data) => {
 let newArr = data.map(item => item).reduce((a ,value)=> a.height > value.height ? a : value);
 return newArr.name;
};
